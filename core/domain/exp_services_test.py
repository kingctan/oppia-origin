...
class UpdateStateTests(ExplorationServicesUnitTests):
    ...
    def test_update_interaction_handlers_fails(self):
        """Test which does a legacy interaction handler update."""
        exploration = exp_services.get_exploration_by_id(self.EXP_ID)
    ...
    def test_update_state_extra_keys(self):
        ...
        # TODO(bhenning): Change this to verify inputs correctly match the rule.
        pass
...
""") % (
    ...
    def test_migration_then_reversion_maintains_valid_exploration(self):
        ...
        new_state_interaction.answer_groups[0].rule_specs[0].dest = 'END'
        init_state_interaction.answer_groups[0].rule_specs[0].dest = 'New state'
        with self.swap(exploration, 'validate', fake_validate):
            exp_services._save_exploration(
                'committer_id_v3', exploration, 'Added new state', [])
